  ...
  onFormSubmit(formData: any) {
    ...
    if (editModel.LocalizedContents !== this.rule.LocalizedContents) {
      const editedLocalizedContentsDictinary: any = editModel.LocalizedContents.reduce(
        (obj: {[key: string]: ActionRuleMobileContent}, item) => {
      ...
      this.rule.LocalizedContents.forEach(mobContent => {
  ...
  private saveImages(formData: SpendActionRule) {
    ...
    if (mobileContentsWithImages.length) {
      const requests = mobileContentsWithImages.map(mobContent => {
        const existingMobContent = this.rule.LocalizedContents.find(val => val.MobileLanguage === mobContent.MobileLanguage);
